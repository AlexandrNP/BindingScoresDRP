...
1. select_features_by_missing_values
2. select_features_by_variation
3. select_decorrelated_features

Pilot 1 specific feature selection functions:
1. coxen_single_drug_gene_selection
2. coxen_multi_drug_gene_selection

Pilot 1 specific data preprocessing functions:
1. quantile_normalization
2. combat_batch_effect_removal
3. generate_gene_set_data

General data preprocessing functions:
1. generate_cross_validation_partition

'''



def write_partition(partition, dir):
    for i in range(len(partition)):
        folder = dir + '/cv_' + str(i)
        if os.path.exists(folder):
            shutil.rmtree(folder)
        os.mkdir(folder)

        pd.DataFrame(partition[i][0], columns=None, index=None).to_csv(folder + '/TrainList.txt', header=False,
                                                                       index=False, sep='\t', line_terminator='\r\n')
        pd.DataFrame(partition[i][1], columns=None, index=None).to_csv(folder + '/ValList.txt', header=False,
                                                                       index=False, sep='\t', line_terminator='\r\n')
        pd.DataFrame(partition[i][2], columns=None, index=None).to_csv(folder + '/TestList.txt', header=False,
                                                                       index=False, sep='\t', line_terminator='\r\n')
...
def generate_gene_set_data(data, genes, gene_name_type='entrez', gene_set_category='c6.all', metric='mean',
    ...
    gene_name_type: string, indicating the type of gene name used in genes. 'entrez' indicates Entrez gene ID and
        'symbol' indicates HGNC gene symbol. Default is 'symbol'.
    ...
    f = open('../Data/Gene_Sets/MSigDB.v7.0/' + gene_set_category, 'r')
    x = f.readlines()
    gene_sets = {}
...
def combat_batch_effect_removal(data, batch, model=None, numerical_covariates=None):
    '''
    ...
    batch: pandas series, with a length of n_samples. It should provide the batch labels of samples.
